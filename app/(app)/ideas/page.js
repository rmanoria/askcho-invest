...
function pickReason(s) {
  if (s.peRatio < 12) return "Trading at a low P/E of " + s.peRatio + " relative to its sector \u2014 looks undervalued.";
  if (s.changePct >= 2) return "Strong momentum today, up " + s.changePct.toFixed(2) + "%, with volume picking up.";
  if (s.changePct <= -2) return "Pulled back " + Math.abs(s.changePct).toFixed(2) + "% \u2014 could be a buy-the-dip setup if the sector holds up.";
  return "Steady performer in " + s.sector + " with a reasonable P/E of " + s.peRatio + ".";
}
...
export default function IdeasPage() {
  ...
  const avgChange = stocks.reduce((a, s) => a + s.changePct, 0) / stocks.length;
    ...
    <>
      ...
      <div className="iv-view">
        ...
        <div className="iv-panel">
          ...
          <p className="iv-sub" style={{ marginBottom: 12 }}>A daily shortlist based on valuation and momentum. Not financial advice \u2014 always do your own research.</p>
